(function() {
  ...
  module.exports = {
     ...
     */
    parseID3v2: function(content) {
      var contentSize, obj, offset;
      obj = {};
      offset = 0;
      
      // ID3v2 Header
      obj.header = this.parseID3v2Header(content, offset);
      // size of extendedHeader + frames + padding
      contentSize = content.length - (obj.header.flags.footerPresent ? 20 : 10);
      if (obj.header.size !== contentSize) {
      ...
      if (obj.header.flags.extendedHeader) {
        obj.extendedHeader = this.parseID3v2ExtendedHeader(content, offset);
      ...
      if (contentSize > 0) {
        ...
        obj.frames = this.parseID3v2Frames(content, offset, contentSize, obj.header.majorVersion >= 4);
        obj.sizeOfTotalFrames = this.sizeOfTotalSizeFrames(obj.frames);
      ...
      if (obj.header.flags.footerPresent) {
        obj.footer = this.parseID3v2Footer(content, offset);
      }
      return obj;
    },
    /*
    3.1.   ID3v2 header
     ...
     $80.
    */
    parseID3v2Header: function(content, offset) {
      var flags, id, majorVersion, minorVersion, size;
      id = content.toString('ascii', offset, offset + 3);
      assert.ok(id === 'ID3', `unknown identifier. Id:[${id}]`);
      offset += 3;
      ...
      offset += 1;
      size = this.fromSyncsafeInt(content, offset, offset + 4);
      return {
        id: id,
    ...
    3.2. Extended header
     ...
     t - Image size restrictions
       ...
       11  All images are exactly 64x64 pixels, unless required
           otherwise.
    */
    parseID3v2ExtendedHeader: function(content, offset) {
      return this.removeUndefinedProperty(this._parseID3v2ExtendedHeader(content, offset));
    },
    _parseID3v2ExtendedHeader: function(content, offset) {
      var crc, flags, length, numberOfFlagBytes, restrictions, size;
      size = this.fromSyncsafeInt(content, offset, 4);
      ...
      assert.ok(numberOfFlagBytes === 1, `Number of flags bytes of extened header should be 1. occurs:${numberOfFlagBytes}`);
      offset += 1;
      flags = content.readUInt8(5);
      offset += 1;
      return {
        size: size,
        tagIsAnUpdate: flags & 0x40 ? (length = content.readUInt8(offset), assert.ok(length === 0, `Flag data length of [Tag is an update] should be 0. occurs:${length}`), offset += 1, true) : void 0,
        crcDataPresent: flags & 0x20 ? (length = content.readUInt8(offset), assert.ok(length === 5, `Flag data length of [CRC data present] should be 5. occurs:${length}`), offset += 1, crc = this.fromSyncsafeInt(content.slice(offset, offset + 5)), offset += 5, {
          crc: crc
        }) : void 0,
        tagRestrictions: flags & 0x10 ? (length = content.readUInt8(offset), assert.ok(length === 1, `Flag data length of [Tag Restrictions] should be 1. occurs:${length}`), offset += 1, restrictions = content.readUInt8(offset), {
          tagSize: ['0 - No more than 128 frames and 1 MB total tag size.', '1 - No more than 64 frames and 128 KB total tag size.', '2 - No more than 32 frames and 40 KB total tag size.', '3 - No more than 32 frames and 4 KB total tag size.'][restrictions >> 6],
          textEncoding: ['0 - No more than 128 frames and 1 MB total tag size.', '1 - No more than 64 frames and 128 KB total tag size.'][restrictions >> 5 & 1],
          textfieldsSize: ['0 - No restrictions', '1 - No string is longer than 1024 characters.', '2 - No string is longer than 128 characters.', '3 - No string is longer than 30 characters.'][restrictions >> 3 & 3],
          imageEncoding: ['0 - No restrictions', '1 - Images are encoded only with PNG [PNG] or JPEG [JFIF].'][restrictions >> 2 & 1],
          imageSize: ['0 - No restrictions', '1 - All images are 256x256 pixels or smaller.', '2 - All images are 64x64 pixels or smaller.', '3 - All images are exactly 64x64 pixels, unless required otherwise.'][restrictions & 3]
        }) : void 0
      };
    },
    /*
    ...
    */
    parseID3v2Frames: function(content, offset, contentSize, useSyncsafeSize) {
      var data, flags, frames, id, results, size, totalSize;
      frames = [];
      totalSize = 0;
      results = [];
      ...
      while ((contentSize - totalSize) >= 10) {
        ...
        offset += 4;
        size = useSyncsafeSize ? this.fromSyncsafeInt(content, offset, offset + 4) : content.readUInt32BE(offset, offset + 4);
        offset += 4;
        flags = [content.readUInt8(offset), content.readUInt8(offset + 1)];
        offset += 2;
    ...
    3.4.   ID3v2 footer
     ...
     ID3v2 size             4 * %0xxxxxxx
    */
    parseID3v2Footer: function(content, offset) {
      var flags, id, majorVersion, minorVersion, size;
      id = (content.slice(offset, 3)).toString();
      assert.ok(id === '3DI', `unknown footer identifier. Id:[${id}]`);
      offset += 3;
      majorVersion = content.readUInt8(offset);
      offset += 1;
      minorVersion = content.readUInt8(offset);
      ...
      offset += 1;
      size = this.fromSyncsafeInt(content.slice(6, 10));
      offset += 4;
      return {
        identifier: id,
        version: `ID3v2.${majorVersion}.${minorVersion}`,
    ...
    sizeOfTotalSizeFrames: function(frames) {
    ...
    toSyncsafeInt: function(v, size) {
      var i;
      return Buffer.from((function() {
        var j, ref, results;
        results = [];
        for (i = j = ref = size - 1; (ref <= 0 ? j <= 0 : j >= 0); i = ref <= 0 ? ++j : --j) {
          results.push(v >> 7 * i & 0x7f);
        }
        return results;
      })());
    },
    fromSyncsafeInt: function(buffer, start, end) {
      if (start === void 0) {
        start = 0;
        end = buffer.length;
      }
      return buffer.slice(start, end).reduce(function(a, c) {
        return (a << 7) + c;
      });
    },
    removeUndefinedProperty: function(o) {
      return Object.keys(o).forEach(function(key) {
        return o[key] === void 0 && delete o[key];
      });
    }
  };
